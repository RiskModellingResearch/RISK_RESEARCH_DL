...
class DenseFeatureLayer(nn.Module):
    ...
    def __init__(self, nrof_cat, emb_dim,
        ...
        for i, col in enumerate(numeric_columns):
            self.first_order_embd[col] = torch.nn.init.xavier_uniform_(torch.empty(1,1))
        ...
        for i, col in enumerate(numeric_columns):
            self.second_order_embd[col] = torch.nn.init.xavier_uniform_(torch.empty(emb_dim, 1))
    ...
    def forward(self, input_data):
        ...
        for i, col in enumerate(self.emb_columns):
            if first_order_embd_output is None:
                first_order_embd_output = self.first_order_embd[col](torch.tensor(input_data[self.emb_columns[i]].detach().numpy(),
                dtype=torch.int64))
            else:
        ...
        for i, col in enumerate(self.emb_columns):
            if second_order_embd_output is None:
                second_order_embd_output = self.second_order_embd[col](
                    torch.tensor(input_data[self.emb_columns[i]].detach().numpy(),
                     dtype=torch.int64))
            else:
                second_order_embd_output = torch.cat(
                    [second_order_embd_output,
                     self.second_order_embd[col](torch.tensor(input_data[self.emb_columns[i]].detach().numpy(),
                     dtype=torch.int64))],
                    dim=1)
